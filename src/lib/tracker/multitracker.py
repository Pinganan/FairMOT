...
class STrack(BaseTrack):
    ...
    def tlwh_to_maplocation(self, pm):
        x1, y1, w, h = self.tlwh
        # center =(int(x1 + 0.5 * w), int(y1 + h))
        center =(x1 + 0.5 * w, y1 + h)
        dot = pm.pixel_to_lonlat(center)
    ...
    def detection_assign_mpaxy(self, x, y):
        self.mapx = x
        self.mapy = y
...
class JDETracker(object):
    def __init__(self, opt, frame_rate=30, port=221):
        ...
        self.pixelmap = PixelMapper(port)
        self.frame_id = 0
    ...
    def map_detection(self, detections):
        for d in detections:
            d.tlwh_to_maplocation(self.pixelmap)
    ...
    def update(self, detections):
        ...
        just_terminal_display(dists, strack_pool, "only motion")
        print()
        ...
        print("    u_track amount " + str(len(u_track)))
        print("    u_det   amount " + str(len(u_detection)))
        just_terminal_display(dists, strack_pool, "tracker.features embedding")
        print()
        ...
        print("    u_det   amount " + str(len(u_detection)))
        just_terminal_display(dists, unconfirmed, "tracker(u) -> stracker(s) iou_distance")
        print()
