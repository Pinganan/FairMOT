...
pixel_path = [deque(maxlen=9999) for _ in range(9999)]
id_color = {}
def plot_pixel(image, map_xy, map_ids):
    #draw mapping path
    image = np.copy(image)
    for dot, track_id in zip(map_xy, map_ids):
        # color = get_color(track_id)
        if not(track_id in id_color):
            count = len(id_color)
            id_color[track_id] = count
        cid = id_color[track_id]
        color = get_color(cid)
        dot = (int(dot[0]), int(dot[1]))
        pixel_path[cid].append(dot)
        cv2.circle(image, dot, 2, color, thickness=2)
        #print(id_color)
        for j in range(1, len(pixel_path[id_color[track_id]])):
            # print('++++++++++++++++++')
            # print(track_id)
            # print(dot)
            if pixel_path[cid][j - 1] is None or pixel_path[cid][j] is None:
               continue
            cv2.line(image,(pixel_path[cid][j-1]), (pixel_path[cid][j]),color,thickness=2)
    return image
